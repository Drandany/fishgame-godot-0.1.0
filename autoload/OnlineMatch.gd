...
func create_match(n_socket: NakamaSocket) -> void:
	if n_socket == null:
		emit_signal("error", "No Nakama socket provided to create_match")
		return
	leave()
	_set_nakama_socket(n_socket)
	match_mode = MatchMode.CREATE

	var data = await n_socket.create_match_async()
...
func join_match(n_socket: NakamaSocket, _match_id: String) -> void:
	leave()
	_set_nakama_socket(n_socket)
...
func start_matchmaking(n_socket: NakamaSocket, data: Dictionary = {}) -> void:
	leave()
	_set_nakama_socket(n_socket)
